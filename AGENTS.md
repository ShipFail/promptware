...
## Responsibilities
1.  **Kernel Development**: Maintain `os/kernel/KERNEL.md` and `os/kernel/bus.ts`. Ensure the Promptware Kernel remains minimal and the Software Kernel remains robust.
...
*   **Software Kernel (`bus.ts`)**: The "Precision" of the OS. Written in pure TypeScript. It defines *how* things happen (I/O, Precision, Determinism).
...
2.  **Monolithic Kernel Architecture**: 
    ...
    *   All tools must be callable via the Unified Entry Point (`bus.ts`).
