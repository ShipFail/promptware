...
Deno.test("RFC 0018: Memory MUST enforce absolute paths on get", async () => {
  await assertRejects(
    async () => await memoryGetModule.handler({ key: "relative/path" }, {} as any),
    Error,
...
Deno.test("RFC 0018: Memory MUST enforce absolute paths on set", async () => {
  await assertRejects(
    async () => await memorySetModule.handler({ key: "relative/path", value: "test" }, {} as any),
    Error,
    "Invalid path: 'relative/path'. Paths MUST be absolute (start with /)."
  );
});
...
Deno.test("RFC 0018: Memory MUST enforce Sealed-at-Rest for /vault/", async () => {
  // Valid pwenc
  await memorySetModule.handler({ key: "/vault/test", value: "pwenc:v1:valid" }, {} as any);

  // Invalid plaintext
  await assertRejects(
    async () => await memorySetModule.handler({ key: "/vault/bad", value: "plaintext_secret" }, {} as any),
    Error,
...
Deno.test("RFC 0018: Memory CRUD operations", async () => {
  ...
  // Set
  await memorySetModule.handler({ key, value: JSON.stringify(value) }, {} as any);

  // Get
  const retrieved = await memoryGetModule.handler({ key }, {} as any);
  assertEquals(typeof retrieved, "string"); // KV stores the JSON string

  // List
  const list = await memoryListModule.handler({ prefix: "/users/test/" }, {} as any);
  assertEquals(list["/users/test/setting"], JSON.stringify(value));

  // Delete
  await memoryDeleteModule.handler({ key }, {} as any);
  const deleted = await memoryGetModule.handler({ key }, {} as any);
...
Deno.test("RFC 0018: Memory MUST store JSON values as strings", async () => {
  ...
  await memorySetModule.handler({ key, value: jsonValue }, {} as any);
  const result = await memoryGetModule.handler({ key }, {} as any);
...
Deno.test("RFC 0018: Memory MUST store non-JSON values as strings", async () => {
  ...
  await memorySetModule.handler({ key, value: plainValue }, {} as any);
  const result = await memoryGetModule.handler({ key }, {} as any);
