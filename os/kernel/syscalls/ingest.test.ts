...
Deno.test("RFC 0020: Ingest MUST return raw content without front matter", async () => {
  ...
  try {
    await withMockKv("https://root.com/", async () => {
      const result = await ingestModule.handler({ uri: "https://example.com/simple.md" }, {} as any);
...
# Hello`;
  ...
  try {
    await withMockKv("https://root.com/", async () => {
      const result = await ingestModule.handler({ uri: "https://example.com/agent.md" }, {} as any);
...
Deno.test("RFC 0020: Ingest MUST fail on fetch error", async () => {
  ...
  try {
    await withMockKv("https://root.com/", async () => {
      await assertRejects(
        async () => await ingestModule.handler({ uri: "https://example.com/bad.md" }, {} as any),
        Error,
        "Network Error"
      );
    });
...
Deno.test("RFC 0020: Ingest MUST preserve body content", async () => {
  ...
  try {
    await withMockKv("https://root.com/", async () => {
      const result = await ingestModule.handler({ uri: "https://example.com/test.md" }, {} as any);
