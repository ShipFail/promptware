...
- **Architecture**: Completed **RFC-23 Dual-Mode Syscall Bridge**.
    ...
    - Added `Syscall.Authenticate` and `Syscall.Shutdown` for secure daemon lifecycle.
- **Protocol**: Refactored `OsEvent` to `OsMessage` (**RFC-0024**).
    - Adopted CQRS schema: `kind` (command/query/event), `type` (topic), `data` (payload).
    - Aligned with Web Worker semantics (`worker.postMessage`).
