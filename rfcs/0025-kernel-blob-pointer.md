...
# RFC 0025: BlobPointer Serialization (Transferable)
...
BlobPointer implements the **Transferable** concept from the W3C Web Worker specification. It allows large data to be "transferred" between the Prompt Kernel (Main Thread) and Software Kernel (Worker) by reference, avoiding the token cost of serialization.
...
2. **Transferable Semantics**: To align with W3C Web Workers, we need a standard way to "transfer" ownership of large data buffers (conceptually) without copying.
3. **NDJSON Compatibility**: Message streams use NDJSON. Inline binary data breaks readability.
4. **Ephemeral Plumbing**: Kernels need to pass intermediate artifacts (e.g., ingested markdown, tool outputs) between components.
5. **Deterministic Serialization**: For testing, logging, and replay, pointer representation must be canonical.
...
* Implement **Transferable** semantics for zero-copy (by reference) messaging.
...
1.  **Ownership Transfer**: Conceptually, the sender releases responsibility for the resource, and the receiver accepts it.
2.  **Zero-Copy**: The data itself is not copied into the message stream. Only the pointer is serialized.
3.  **Dereference**: The receiver is responsible for dereferencing the pointer (reading the file, fetching the URL) if it needs the content.

**The Mental Model**:
> "I am sending you this file handle. You can read it now."
...
##### 5.5.3.1 Size Policy for `data` Scheme
...
#### 5.6.2 `https` Authority Normalization
...
* scheme-specific constraints from §5.5.
...
### 8.1 Why Decomposed URI Components?
...
### 13.1 Local Temporary Markdown Output

```json
{ "scheme": "file", "path": "/tmp/pwo/ingest/2025-12-30/out.md" }
```

### 13.2 Cloud Blob via HTTPS with Query Parameters

```json
{ "scheme": "https", "authority": "blob.example.com", "path": "/tmp/out.md", "query": "sig=abc&exp=1730000000" }
```

### 13.3 Citing a Slice (Non-Normative Fragment Examples)
...
* [W3C Web Workers](https://html.spec.whatwg.org/multipage/workers.html)
...
* **Transferable**: A W3C concept for zero-copy data transfer
